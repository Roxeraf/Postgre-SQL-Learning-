...
function insertColumnIntoSelect(editor, col) {
  ...
  if (new RegExp(`\\b${col}\\b`, "i").test(existing)) {
    flashEditor(editor);
    return true;
  }
  let newList;
  ...
  flashEditor(editor);
  const status = document.getElementById("schema-rail-status");
  if (status) {
    status.hidden = false;
    status.textContent = `${col} nach SELECT eingefügt`;
  }
  return true;
}
...
function insertAtCursor(text, editorEl) {
  ...
  if (ident && insertColumnIntoSelect(editor, text)) return;
...
function initSchema() {
  ...
  const renderDrawer = () => {
    ...
    list.innerHTML = filtered.map((t) => tableCard(t, false)).join("") || '<p class="muted">Kein Treffer.</p>';
  };
  ...
  const renderRail = () => {
    ...
    railList.innerHTML = filtered.map((t) => tableCard(t, Boolean(shorts))).join("") || '<p class="muted">Keine Tabellen zu dieser Übung.</p>';
    if (railMeta) {
...
const PHASE_HINTS = {
  ...
  practice: "Eine Aufgabe. Rechts die Tabelle. Spalte klicken, ausführen, dann „Stimmt das?“.",
